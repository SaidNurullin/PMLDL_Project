...
import os
import pickle
# pritty_midi keras tensorflow

def generate_music(network_input, note_encoder, instrument_encoder, n_vocab, model, starts_scaler, durations_scaler, velocity_scaler, instruments, num_notes_to_generate, output_file):
    # Create MIDI data
...
# path_to_models = os.path.dirname(path_to_models)
# path_to_models = os.path.dirname(path_to_models)
# path_to_models = os.path.dirname(path_to_models)
path_to_models = os.path.join(path_to_models, 'models')
models_dir = os.path.join(path_to_models, 'model')
encoders_dir = os.path.join(path_to_models, 'encoders')
scalers_dir = os.path.join(path_to_models, 'scalers')
inputs_dir = os.path.join(path_to_models, 'input')
n_vocabs_dir = os.path.join(path_to_models, 'n_vocab')
instruments_dir = os.path.join(path_to_models, 'instruments')
...
def generate(length, instruments, output_file):
    print(instruments)
    num_notes_to_generate = int(length * 4)
    generate_music(network_input, encoders[1], encoders[0], n_vocab, model, scalers[1], scalers[0], scalers[2], instruments, num_notes_to_generate, output_file)